...
public class LogoSettingsActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.run.sg.amap3d.R.layout.logo_settings_activity);
		/*
	...
	private void init() {
		...
		RadioGroup radioGroup = (RadioGroup) findViewById(com.run.sg.amap3d.R.id.logo_position);
	...
	public void onCheckedChanged(RadioGroup group, int checkedId) {
		if (aMap != null) {
			if (checkedId == com.run.sg.amap3d.R.id.bottom_left) {
				mUiSettings
						.setLogoPosition(AMapOptions.LOGO_POSITION_BOTTOM_LEFT);// 设置地图logo显示在左下方
			} else if (checkedId == com.run.sg.amap3d.R.id.bottom_center) {
				mUiSettings
						.setLogoPosition(AMapOptions.LOGO_POSITION_BOTTOM_CENTER);// 设置地图logo显示在底部居中
			} else if (checkedId == com.run.sg.amap3d.R.id.bottom_right) {
				mUiSettings
